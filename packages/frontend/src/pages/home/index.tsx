import { ReactElement, memo, useRef, useState, useEffect, useContext } from 'react'
...
const Home: FC<IProps> = memo(function (props) {
  ...
  useEffect(() => {
    socket?.on('message', async (data) => {
      if(data.to !== userInfo.name || data.send !== friendList.find(val => val.id === isActiveId)!.name ) return
      await dispatch(
        fetchMessageList({
  ...
  const onSearch = async (value: string, e: any) => {
    if (value == '') {
      ...
      socket?.emit('message',{
        to:friendList.find(val => val.id === isActiveId)!.name,
        send:userInfo.name
      })
      setMessageIpt('')
    }
